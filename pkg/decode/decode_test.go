...
func TestMasterConfig(t *testing.T) {
	...
	}{
		{
			...
			expectedMasterConfig: &legacyconfigv1.MasterConfig{
				...
				ImagePolicyConfig: legacyconfigv1.ImagePolicyConfig{
					...
					AllowedRegistriesForImport:                 nil, //&legacyconfigv1.AllowedRegistries{},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			content, err := ioutil.ReadFile(tc.filename)
			if err != nil {
				t.Fatalf("Cannot read file %s", tc.filename)
			}
			masterConfig, err := MasterConfig(content)
			require.NoError(t, err)

			assert.Equal(t, masterConfig, tc.expectedMasterConfig)
		})
	}
}
